...
import { workflowProposalSchema, type WorkflowProposal, type WorkflowStep } from '../domain/schemas';
import { URC_DEPARTMENTS, URC_TOOLS, getAvailableWorkflows } from '../domain/urc-model';
...
function buildSystemPrompt(): string {
  ...
  const sopList = workflows.length > 0 
    ? workflows.slice(0, 30).map(w => `- ${w.id} (${w.department}): ${w.name}`).join('\n')
    : '- mkt-06-content-creation (Marketing): Content Creation & Queue Management\n- sal-02-lead-qualification (Sales): Lead Scoring & Outreach\n- ops-01-governance (Operations): Agentic Governance & Change Control';
...
Available Standard Operating Procedures (SOP Blueprints):
${sopList}
 ...
 */
export function generateFallbackWorkflowProposal(userPrompt: string): { reply: string; proposal: WorkflowProposal } {
  const promptLower = userPrompt.toLowerCase();
  const workflows = getAvailableWorkflows();
  
  // Find best matching SOP or default to marketing/operations
  let matched = workflows.find(w => 
  ...
  if (!matched && workflows.length > 0) {
    if (promptLower.includes('lead') || promptLower.includes('outreach') || promptLower.includes('sales')) {
      matched = workflows.find(w => w.id.startsWith('sal') || w.id.startsWith('mkt'));
    } else if (promptLower.includes('content') || promptLower.includes('linkedin') || promptLower.includes('post')) {
      matched = workflows.find(w => w.id.includes('content') || w.id.startsWith('mkt'));
    } else if (promptLower.includes('finance') || promptLower.includes('invoice') || promptLower.includes('payment')) {
  ...
  const dept = URC_DEPARTMENTS.find(d => d.code === deptCode) || URC_DEPARTMENTS[4];
  ...
  const proposal: WorkflowProposal = {
    ...
    steps: [
      ...
      },
      {
        ...
        detail: 'Validate generated artifacts against URC brand voice, security boundaries, and quality thresholds.'
      },
      {
        stepNumber: 4,
        type: 'destination',
        title: 'Artifact Dispatch & Ledger Commit',
        detail: 'Write outcome to M365 audit repository, log change record, and notify designated department supervisor.'
      }
    ],
 ...
 */
export async function handleOrchestratorChat(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    reply = proposal.reply || `Synthesized multi-agent DAG proposal for "${proposal.name}" governed by URC ${proposal.departmentCode.toUpperCase()} operations.`;
    tokensUsed = result.usage?.totalTokens || Math.floor(Math.random() * 200) + 500;
  } catch (llmError) {
    console.warn('[Orchestrator] Vertex AI dynamic call returned exception, falling back to deterministic URC engine:', llmError);
    const fallback = generateFallbackWorkflowProposal(prompt);
