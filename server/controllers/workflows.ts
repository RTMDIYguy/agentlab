...
const DEFAULT_WORKSPACE_WORKFLOWS: WorkflowSummaryDto[] = [
  {
    ...
    description: 'Enriches founder leads from Bootstrapper events with LinkedIn and Hunter.io APIs.',
  ...
  },
  {
    ...
    description: 'Reconciles Stripe transactions against M365 Finance Control sheet at 18:00 UTC.',
  ...
  },
  {
    ...
    description: 'Runs Vitest suites, checks TypeScript compilation, and submits PR reports.',
...
export async function getWorkflows(req: Request, res: Response): Promise<void> {
  try {
    ...
    res.status(200).json({
      workspaceId,
      workflows: DEFAULT_WORKSPACE_WORKFLOWS,
      totalCount: DEFAULT_WORKSPACE_WORKFLOWS.length
    });
  } catch (error) {
...
export async function deployWorkflow(req: Request, res: Response): Promise<void> {
  try {
    const workspaceId = req.workspaceId;
    const { proposalId, proposal } = req.body as { proposalId?: string; proposal?: ProposedWorkflow };

    if (!proposalId && !proposal) {
      res.status(400).json({ error: 'Either proposalId or full proposal object is required.' });
      return;
    }

    const workflowName = proposal?.name || `Workflow-${proposalId || Date.now()}`;
