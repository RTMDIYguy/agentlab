...
function recommend(answers: Record<string, string>): {
  ...
  if (goal === "revenue" && skill === "writing") {
    return {
      ...
      topics: [
        "Positioning your writing services",
        "Cold outreach that actually works",
        "Pricing without undercharging",
        "Landing your first 3 clients",
      ],
...
export default function Bootcamp() {
  ...
  const progress =
    step === 0 ? 0 : Math.min((step / totalQuizSteps) * 100, 100);
      ...
      <section className="bg-gradient-to-br from-violet-900 to-indigo-900 text-white py-16 px-4">
        ...
        >
          ...
          <p className="text-lg text-indigo-200">
            Find the exact business track built for where you are right now — in
            2 minutes.
          </p>
        </motion.div>
      </section>
          ...
          {step > 0 && step <= totalQuizSteps && (
            <div className="mb-6">
              <div className="flex justify-between text-xs text-muted-foreground mb-2">
                <span>
                  Question {step} of {totalQuizSteps}
                </span>
            ...
            {step === 0 && (
              ...
              >
                ...
                <h2 className="text-2xl font-bold mb-3">
                  Find Your Business Track
                </h2>
                <p className="text-muted-foreground mb-8">
                  Answer 4 quick questions and we'll match you with the bootcamp
                  curriculum built for your situation, skills, and goals.
                </p>
                ...
                <div className="space-y-3">
                  {currentQuizStep.options.map(opt => (
                    <button
                      key={opt.value}
                      onClick={() =>
                        selectOption(currentQuizStep.id, opt.value)
                      }
              ...
              >
                <Card>
                  <CardHeader className="text-center">
                    ...
                    <CardDescription>
                      Enter your email to unlock your personalized curriculum —
                      free.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                ...
                <Card className="border-violet-300 bg-violet-50/50 dark:bg-violet-900/10">
                  <CardContent className="pt-6">
                    ...
                    <p className="text-sm text-muted-foreground mb-4">
                      Take the next step with the entry-level bootcamp before
                      moving into Ownable OS or a URC workflow engagement.
                    </p>
                    <Button
                      className="w-full bg-violet-600 hover:bg-violet-700"
                      onClick={() => createBootcampCheckout.mutate()}
                      disabled={createBootcampCheckout.isPending}
                    >
                      {createBootcampCheckout.isPending ? (
                        <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                      ) : null}
                      Start the $1 Bootcamp
                    </Button>
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardContent className="pt-6 text-center">
                    ...
                    <Button
                      variant="outline"
                      onClick={() =>
                        window.open("https://calendly.com/thebossrob", "_blank")
                      }
                    >
                      Schedule a Call
                      <ArrowRight className="h-4 w-4 ml-2" />
