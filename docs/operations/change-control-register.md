...
| 2026-05-16 | CC-2026-05-16-001 | Documentation / Workflow Packaging | `README.md`; `docs/operations/change-control-register.md`; `docs/operations/scheduled-change-queue.md` | Updated the root README to reflect the workflow registry and source-import architecture, including where to find the active package and packaging template. | Recent workflow import commits significantly changed user-facing navigation paths, while the root README still described an early repo state. | Updated: root-level repo orientation now points to current workflow source-of-truth docs and package paths. | Documentation only | oz | Active |
