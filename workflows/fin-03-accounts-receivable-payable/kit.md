---
schema: kit/1.0
slug: fin-03-accounts-receivable-payable
title: "FIN-03 - Accounts Receivable & Payable"
summary: "Finance workflow package for Accounts Receivable & Payable. Consolidates registry metadata, imported source references, setup guidance, validation notes, and automation blueprints into one operative kit file."
version: 0.1.0-draft
...
department: "Finance"
automationLevel: "85%"
primaryOwner: "Robert + Finance"
trigger: "Milestone complete (AR) or invoice received (AP)"
cycleTime: "Invoice: same day; AP runs: weekly"
...
# FIN-03 - Accounts Receivable & Payable
...
Run the Accounts Receivable & Payable workflow for Finance with a repeatable trigger, owner, cycle time, outputs, and validation path. The current draft is strong enough for agent orientation and controlled internal use; certification still requires a live run or documented manual evidence.

## When to Use

Use this workflow when the trigger occurs: **Milestone complete (AR) or invoice received (AP)**.
...
| Department | Finance |
| Automation level | 85% |
| Owner | Robert + Finance |
| Trigger | Milestone complete (AR) or invoice received (AP) |
| Cycle time | Invoice: same day; AP runs: weekly |
...
1. Confirm the trigger: Milestone complete (AR) or invoice received (AP).
...
| `automation/FIN03-AR-AP-Blueprint.json` | Retained provenance/input artifact now referenced by this consolidated kit. |
| `trackers/FIN03_AR_AP_Tracker.xlsx` | Retained provenance/input artifact now referenced by this consolidated kit. |
...
### automation/FIN03-AR-AP-Blueprint.json

````json
{
  "name": "FIN03: AR/AP Aging Monitor & Reminder (Make.com Blueprint)",
  "flow": [
    {
      "id": 1,
    ...
    },
    {
      ...
      "mapper": {
        "messages": [
          {
            "role": "system",
            "content": "You are a cash flow analyst. Given open AR invoices and upcoming AP bills, classify each as: OVERDUE (past due), DUE_SOON (within 7 days), SCHEDULED (8-14 days). For AR items, draft a brief, professional payment reminder. Deduplicate AP items by vendor+invoice_number+amount. Return JSON: {ar: [{client, amount, daysPastDue, status, reminderDraft}], ap: [{vendor, amount, dueDate, status, isDuplicate}]}."
          },
          {
            "role": "user",
            "content": "AR Invoices: {{2.invoices}}\nAP Bills: {{3.bills}}"
          }
        ]
      },
      "metadata": {
    ...
    },
    {
      ...
      "parameters": {},
      "mapper": {
        "to": "{{4.choices[0].message.content.ar[].client.email}}",
        "subject": "Payment Reminder — Invoice {{4.choices[0].message.content.ar[].invoiceId}}",
        "body": "{{4.choices[0].message.content.ar[].reminderDraft}}"
      },
      "metadata": {
        "designer": {
          "x": 1200,
    ...
    },
    {
      ...
      "metadata": {
        ...
        "note": "Route: any AP status == DUE_SOON (Friday AP run prep)"
      }
    }
  ],
  "metadata": {
...
- `trackers/FIN03_AR_AP_Tracker.xlsx`: binary attachment retained outside the Markdown kit; convert to CSV/Markdown during certification if the sheet contents become authoritative.

## Automation Blueprint

### FIN03-AR-AP-Blueprint.json
