---
schema: kit/1.0
slug: fin-05-investment-savings
title: "FIN-05 - Investment & Savings"
summary: "Finance workflow package for Investment & Savings. Consolidates registry metadata, imported source references, setup guidance, validation notes, and automation blueprints into one operative kit file."
version: 0.1.0-draft
...
cycleTime: "Transfers: monthly; monitoring: daily"
...
Run the Investment & Savings workflow for Finance with a repeatable trigger, owner, cycle time, outputs, and validation path. The current draft is strong enough for agent orientation and controlled internal use; certification still requires a live run or documented manual evidence.

## When to Use

Use this workflow when the trigger occurs: **1st business day of month; drift detected**.
...
1. Confirm the trigger: 1st business day of month; drift detected.
  ...
  "flow": [
    {
      ...
      "parameters": {
        "interval": "1d",
        "time": "08:00",
        "note": "Primary action on 1st business day; daily monitoring otherwise"
      },
      "mapper": {},
      "metadata": {
        "designer": {
    ...
    {
      "id": 2,
      "module": "plaid:getAccountBalances",
      "version": 1,
      "parameters": {
        "accounts": ["operating", "tax", "profit"]
      },
      "mapper": {},
      "metadata": {
        "designer": {
    ...
    },
    {
      ...
      "mapper": {
        "messages": [
          {
            "role": "system",
            "content": "You are a financial planning assistant. Given account balances and the monthly fixed costs figure, calculate: (1) operating reserve coverage in months, (2) whether the profit transfer has been executed this month, (3) whether the Plaid connection is healthy. Flag: RESERVE_LOW if coverage < 3 months, TRANSFER_PENDING if no profit transfer yet this month and today is after the 3rd, PLAID_ERROR if any account shows stale data. Return JSON: {reserveMonths: number, transferStatus: string, plaidHealth: string, alerts: string[]}."
          },
          {
            "role": "user",
            "content": "Balances: {{2.accounts}}\nMonthly Fixed Costs: $8500\nToday: {{formatDate(now; 'YYYY-MM-DD')}}"
          }
        ]
      },
      "metadata": {
    ...
    },
    {
      ...
      "mapper": {
        "text": "💰 *Investment & Savings Alert*\n*Reserve Coverage:* {{3.choices[0].message.content.reserveMonths}} months\n*Transfer Status:* {{3.choices[0].message.content.transferStatus}}\n*Alerts:* {{3.choices[0].message.content.alerts}}"
      },
      "metadata": {
        "designer": {
          "x": 1200,
...
- `trackers/FIN05_Investment_Savings_Tracker.xlsx`: binary attachment retained outside the Markdown kit; convert to CSV/Markdown during certification if the sheet contents become authoritative.

## Automation Blueprint

### FIN05-Investment-Savings-Blueprint.json
...
- `automation/FIN05-Investment-Savings-Blueprint.json`: YOUR_FIN05_TRACKER_ID
