---
schema: kit/1.0
slug: fin-04-accounting-auditing
title: "FIN-04 - Accounting & Auditing"
summary: "Finance workflow package for Accounting & Auditing. Consolidates registry metadata, imported source references, setup guidance, validation notes, and automation blueprints into one operative kit file."
version: 0.1.0-draft
...
cycleTime: "Month-end close: within 5 business days"
...
Run the Accounting & Auditing workflow for Finance with a repeatable trigger, owner, cycle time, outputs, and validation path. The current draft is strong enough for agent orientation and controlled internal use; certification still requires a live run or documented manual evidence.

## When to Use

Use this workflow when the trigger occurs: **Last business day of month**.
...
1. Confirm the trigger: Last business day of month.
...
| `automation/FIN04-Accounting-Auditing-Blueprint.json` | Retained provenance/input artifact now referenced by this consolidated kit. |
| `trackers/FIN04_Accounting_Auditing_Tracker.xlsx` | Retained provenance/input artifact now referenced by this consolidated kit. |
...
````markdown
# FIN-04 - Accounting & Auditing Department Source
...
### automation/FIN04-Accounting-Auditing-Blueprint.json

````json
{
  "name": "FIN04: Month-End Close & Reconciliation (Make.com Blueprint)",
  "flow": [
    {
      "id": 1,
      ...
      "parameters": {
        "interval": "1d",
        "time": "07:00",
        "note": "Active from the 25th through the 5th of next month"
      },
      "mapper": {},
      "metadata": {
        "designer": {
    ...
    {
      "id": 2,
      "module": "quickbooks:getUncategorizedTransactions",
      "version": 1,
      "parameters": {},
      "mapper": {},
      "metadata": {
    ...
    {
      "id": 3,
      "module": "quickbooks:getBankReconciliation",
      "version": 1,
      "parameters": {},
      "mapper": {
        "period": "currentMonth"
    ...
    },
    {
      ...
      "mapper": {
        "messages": [
          {
            "role": "system",
            "content": "You are an accounting close specialist. Given uncategorized transaction count and bank reconciliation status, determine month-end close readiness. Rules: uncategorized count must be 0 to proceed; reconciliation difference must be $0.00. Return JSON: {closeReady: boolean, uncategorizedCount: number, reconDifference: number, blockers: string[], nextAction: string}."
          },
          {
            "role": "user",
            "content": "Uncategorized: {{2.count}}\nReconciliation Difference: {{3.difference}}\nDate: {{formatDate(now; 'YYYY-MM-DD')}}"
          }
        ]
      },
      "metadata": {
    ...
    },
    {
      ...
      "mapper": {
        "text": "⚠️ *Month-End Close Blockers*\n*Uncategorized:* {{4.choices[0].message.content.uncategorizedCount}}\n*Recon Difference:* ${{4.choices[0].message.content.reconDifference}}\n*Next Action:* {{4.choices[0].message.content.nextAction}}"
      },
      "metadata": {
        "designer": {
          "x": 1200,
    ...
    },
    {
      ...
      "mapper": {
        "to": "cpa@example.com",
        "subject": "Month-End Close Complete — Financial Health Pack Ready",
        "body": "The month-end close for {{formatDate(now; 'MMMM YYYY')}} is complete. P&L and Balance Sheet are available in the Finance Department folder."
      },
      "metadata": {
        "designer": {
          "x": 1200,
...
- `trackers/FIN04_Accounting_Auditing_Tracker.xlsx`: binary attachment retained outside the Markdown kit; convert to CSV/Markdown during certification if the sheet contents become authoritative.

## Automation Blueprint

### FIN04-Accounting-Auditing-Blueprint.json
