---
schema: kit/1.0
slug: fin-02-taxes-fees
title: "FIN-02 - Taxes & Fees"
summary: "Finance workflow package for Taxes & Fees. Consolidates registry metadata, imported source references, setup guidance, validation notes, and automation blueprints into one operative kit file."
version: 0.1.0-draft
...
department: "Finance"
automationLevel: "70%"
primaryOwner: "Robert + CPA"
trigger: "30/15/7 days before deadline"
cycleTime: "Quarterly estimated payments; annual filing"
...
Run the Taxes & Fees workflow for Finance with a repeatable trigger, owner, cycle time, outputs, and validation path. The current draft is strong enough for agent orientation and controlled internal use; certification still requires a live run or documented manual evidence.

## When to Use

Use this workflow when the trigger occurs: **30/15/7 days before deadline**.
...
| Owner | Robert + CPA |
| Trigger | 30/15/7 days before deadline |
| Cycle time | Quarterly estimated payments; annual filing |
...
1. Confirm the trigger: 30/15/7 days before deadline.
...
| `automation/FIN02-Taxes-Fees-Blueprint.json` | Retained provenance/input artifact now referenced by this consolidated kit. |
| `trackers/FIN02_Taxes_Fees_Tracker.xlsx` | Retained provenance/input artifact now referenced by this consolidated kit. |
...
````markdown
# FIN-02 - Taxes & Fees Department Source
...
### automation/FIN02-Taxes-Fees-Blueprint.json

````json
{
  "name": "FIN02: Tax Deadline & Payment Tracker (Make.com Blueprint)",
  "flow": [
    {
      "id": 1,
    ...
    },
    {
      ...
      "version": 1,
      "parameters": {
        "spreadsheetId": "YOUR_FIN02_TRACKER_ID",
        "sheetName": "Deadlines"
      },
      "mapper": {
        "filter": "dueDate <= TODAY + 30"
      },
      "metadata": {
        "designer": {
          "x": 300,
    ...
    },
    {
      ...
      "mapper": {
        "messages": [
          {
            "role": "system",
            "content": "You are a tax compliance assistant. Given upcoming tax deadlines, determine which are within 7 days (URGENT), within 15 days (UPCOMING), or within 30 days (PLANNED). Return JSON array with fields: {obligation, dueDate, status, daysRemaining, estimatedAmount}."
          },
          {
            "role": "user",
            "content": "Today: {{formatDate(now; 'YYYY-MM-DD')}}\nDeadlines: {{2.rows}}"
          }
        ]
      },
      "metadata": {
    ...
    },
    {
      ...
      "mapper": {
        "text": "🚨 *URGENT Tax Deadline*\n{{3.choices[0].message.content}}\n*Action:* Execute EFTPS/state payment before deadline."
      },
      "metadata": {
        "designer": {
          "x": 1200,
        ...
        "note": "Route: any item status == URGENT"
      }
    },
    {
      "id": 6,
      "module": "outlook:createEvent",
      "version": 1,
      "parameters": {},
      "mapper": {
        "subject": "Tax Payment Due: {{3.choices[0].message.content}}",
      ...
      "metadata": {
        ...
        "note": "Route: status == UPCOMING (create calendar reminder)"
      }
    }
  ],
  "metadata": {
...
- `trackers/FIN02_Taxes_Fees_Tracker.xlsx`: binary attachment retained outside the Markdown kit; convert to CSV/Markdown during certification if the sheet contents become authoritative.

## Automation Blueprint

### FIN02-Taxes-Fees-Blueprint.json
...
- `automation/FIN02-Taxes-Fees-Blueprint.json`: YOUR_FIN02_TRACKER_ID
