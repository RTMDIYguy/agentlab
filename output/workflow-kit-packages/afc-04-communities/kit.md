---
schema: kit/1.0
slug: afc-04-communities
title: "AFC-04 - Communities"
summary: "AfterCare workflow package for Communities. Consolidates registry metadata, imported source references, setup guidance, validation notes, and automation blueprints into one operative kit file."
version: 0.1.0-draft
...
primaryOwner: "Marcus (content) + Robert (live)"
trigger: "New member joins platform"
cycleTime: "Onboarding: 24 hrs; events: monthly; content: daily"
...
Run the Communities workflow for AfterCare with a repeatable trigger, owner, cycle time, outputs, and validation path. The current draft is strong enough for agent orientation and controlled internal use; certification still requires a live run or documented manual evidence.

## When to Use

Use this workflow when the trigger occurs: **New member joins platform**.
...
| Owner | Marcus (content) + Robert (live) |
| Trigger | New member joins platform |
| Cycle time | Onboarding: 24 hrs; events: monthly; content: daily |
...
1. Confirm the trigger: New member joins platform.
...
| `automation/AFC04-Blueprint.json` | Retained provenance/input artifact now referenced by this consolidated kit. |
| `trackers/AFC-04_Community_Tracker.xlsx` | Retained provenance/input artifact now referenced by this consolidated kit. |
...
````markdown
# AFC-04 - Communities Department Source
...
### automation/AFC04-Blueprint.json

````json
{
  "name": "AFC04: Communities (Make.com Blueprint)",
  "flow": [
    {
      "id": 1,
      "module": "discord:watchMessages",
      "version": 1,
      "parameters": {
        "channel": "general"
      }
    },
    {
      ...
      "mapper": {
        "messages": [
          {
            "role": "system",
            "content": "Monitor community sentiment and flag support requests."
          }
        ]
      }
    },
...
- `trackers/AFC-04_Community_Tracker.xlsx`: binary attachment retained outside the Markdown kit; convert to CSV/Markdown during certification if the sheet contents become authoritative.

## Automation Blueprint

### AFC04-Blueprint.json
