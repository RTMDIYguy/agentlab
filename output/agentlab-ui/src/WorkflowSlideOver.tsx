...
export const WorkflowSlideOver: React.FC<WorkflowSlideOverProps> = ({
  workflow,
  onClose,
}) => {
      ...
      <div className="fixed inset-y-0 right-0 w-full max-w-xl bg-navy-900 border-l border-navy-800 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col">
        ...
        <div className="flex items-start justify-between p-6 border-b border-navy-800/50">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h2 className="font-poppins text-2xl font-semibold text-white tracking-wide">
                {workflow.name}
              </h2>
              <span
                className={`px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider rounded-full border ${statusStyles[workflow.status]}`}
              >
                {workflow.status}
              </span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-navy-950/50 p-4 rounded-xl border border-navy-800/50">
              ...
              <p className="font-poppins text-lg font-medium text-slate-200">
                {workflow.lastRun}
              </p>
            </div>
            <div className="bg-navy-950/50 p-4 rounded-xl border border-navy-800/50">
              ...
              <p className="font-poppins text-lg font-medium text-cyan-400">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-poppins text-sm font-medium text-slate-200 uppercase tracking-widest">
                Assigned Agents
              </h3>
            ...
            <div className="bg-navy-950/50 rounded-xl border border-navy-800/50 divide-y divide-navy-800/50">
              <div className="p-4 flex items-center justify-between group">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-inter text-sm font-medium text-white group-hover:text-cyan-400 transition-colors">
                      Alpha-Node-01
                    </p>
            ...
            <div className="bg-black border border-navy-800 rounded-lg p-4 font-mono text-xs leading-loose overflow-x-auto whitespace-pre">
              <div className="flex gap-4">
                ...
                <span className="text-slate-300">
                  Execution #4092 completed in 1m 45s. Processed 120 items.
                </span>
              </div>
              <div className="flex gap-4">
                ...
                <span className="text-green-400">[SUCCESS]</span>
                <span className="text-slate-300">
                  Execution #4091 completed in 2m 12s. Processed 154 items.
                </span>
              </div>
              <div className="flex gap-4">
        ...
        <div className="bg-navy-950/50 p-6 border-t border-navy-800/50 flex justify-end gap-3 mt-auto">
          {workflow.status === "running" ? (
            <Button
              variant="secondary"
              className="text-amber-400 hover:text-amber-400 hover:border-amber-400/50"
            >
              Pause Workflow
            </Button>
          ) : (
            <Button
              variant="secondary"
              className="text-cyan-400 hover:text-cyan-400 hover:border-cyan-400/50"
            >
              Start Workflow
            </Button>
          )}
