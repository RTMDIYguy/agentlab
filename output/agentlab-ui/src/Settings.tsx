    ...
    setApiKeys(prev => prev.map(k => {
      if (k.id === id) {
        return {
          ...k,
          value: val,
          masked: val ? `${val.slice(0, 8)}••••••••${val.slice(-4)}` : 'Not configured',
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="font-inter text-sm text-slate-400 mt-1">
          Configure API credentials, runtime budget guardrails, security policies, and workspace parameters.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <div className="md:col-span-4 lg:col-span-3 space-y-1">
          <nav className="bg-navy-900 border border-navy-800 rounded-2xl p-2 space-y-1">
            {[
              ...
              { id: 'apikeys', label: 'API Keys & Providers', icon: 'M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z' },
              { id: 'billing', label: 'Billing & Budget', icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
              { id: 'security', label: 'Security & Guardrails', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
              { id: 'team', label: 'Team & Access', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z' }
            ].map((tab) => (
              <button
                key={tab.id}
        ...
        <div className="md:col-span-8 lg:col-span-9">
          <form onSubmit={handleSave} className="bg-navy-900 border border-navy-800 rounded-2xl p-6 md:p-8 space-y-6">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="font-inter text-xs text-slate-400 mt-1">
                    Keys are stored securely and injected dynamically into sandboxed agent runtimes.
                  </p>
                </div>

                <div className="space-y-4">
                  {apiKeys.map((keyItem) => (
                    <div key={keyItem.id} className="bg-navy-950/70 p-4 rounded-xl border border-navy-800/80 space-y-2">
                      <div className="flex items-center justify-between">
                        <div>
                          <span className="text-xs font-semibold font-poppins text-white">{keyItem.name}</span>
                          <span className="text-[11px] font-inter text-slate-500 ml-2">({keyItem.provider})</span>
                        </div>
            ...
            {activeTab === 'billing' && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="font-inter text-xs text-slate-400 mt-1">
                    Control maximum financial exposure and configure autonomic pause triggers.
                  </p>
                </div>
                ...
                <div className="bg-navy-950/70 p-5 rounded-xl border border-navy-800/80 space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-xs font-semibold font-poppins text-white">Hard Monthly Spend Cap</div>
                      <div className="text-[11px] font-inter text-slate-400">Total allowed LLM inference & scraping expenditure per month.</div>
                    </div>
                ...
                <div className="bg-navy-950/70 p-5 rounded-xl border border-navy-800/80 flex items-center justify-between">
                  <div>
                    <div className="text-xs font-semibold font-poppins text-white">Auto-Pause Swarm at 90% Threshold</div>
                    <div className="text-[11px] font-inter text-slate-400">Gracefully halt new cron triggers when expenditure exceeds ${(monthlyLimit * 0.9).toFixed(0)}.</div>
                  </div>
            ...
            {activeTab === 'security' && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="font-inter text-xs text-slate-400 mt-1">
                    Manage data privacy, SAIF compliance policies, and audit retention schedules.
                  </p>
                </div>

                <div className="bg-navy-950/70 p-5 rounded-xl border border-navy-800/80 flex items-center justify-between">
                  <div>
                    <div className="text-xs font-semibold font-poppins text-white">Automatic PII Redaction</div>
                    <div className="text-[11px] font-inter text-slate-400">Filter SSNs, phone numbers, and private credentials before sending to LLMs.</div>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                ...
                <div className="bg-navy-950/70 p-5 rounded-xl border border-navy-800/80 flex items-center justify-between">
                  <div>
                    <div className="text-xs font-semibold font-poppins text-white">SAIF Compliance Enforcement</div>
                    <div className="text-[11px] font-inter text-slate-400">Enforce Secure AI Framework zero-trust validation on external tool execution.</div>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                ...
                <div className="bg-navy-950/70 p-5 rounded-xl border border-navy-800/80 space-y-2">
                  <div className="text-xs font-semibold font-poppins text-white">Audit Log Retention Period</div>
                  <select
                    value={retentionDays}
                  ...
                  >
                    ...
                    <option value="90">90 Days (Recommended for URC Operations)</option>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-poppins text-slate-300 font-medium">Workspace Organization</label>
                    <input
                      type="text"
                      value={workspaceName}
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-poppins text-slate-300 font-medium">Default Orchestrator Model</label>
                    <select
                      value={defaultModel}
                ...
                <div className="space-y-3">
                  <div className="p-4 bg-navy-950/70 rounded-xl border border-navy-800 flex items-center justify-between text-xs font-inter">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-white font-medium">Robert M. (You)</div>
                        <div className="text-slate-500 font-mono text-[11px]">Owner &bull; Full Admin Privileges</div>
                      </div>
                    </div>
