...
export const AuditDetailSlideOver: React.FC<AuditDetailSlideOverProps> = ({
  log,
  onClose,
}) => {
  if (!log) return null;
        ...
        <div className="flex items-start justify-between p-6 border-b border-navy-800/80 bg-navy-950/40">
          <div>
            <div className="flex items-center gap-3 mb-1.5">
              <span
                className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-mono font-medium border ${currentStatus.badge}`}
              >
                <span
            ...
            <h2 className="font-poppins text-lg font-semibold text-white tracking-wide">
              {log.actionType} &middot;{" "}
              <span className="text-cyan-300">{log.agentName}</span>
            </h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="bg-navy-950/60 p-3.5 rounded-xl border border-navy-800/80">
              ...
              <div
                className="text-xs font-mono font-semibold text-white truncate"
                title={log.model}
              >
                {log.model}
              </div>
          ...
          <div className="bg-navy-950/40 p-4 rounded-xl border border-navy-800/60 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-xs font-inter">
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-navy-900/80 border border-navy-800">
                ...
                <span
                  className={`font-mono font-medium ${log.policyChecks.saifPassed ? "text-emerald-400" : "text-rose-400"}`}
                >
              ...
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-navy-900/80 border border-navy-800">
                ...
                <span className="font-mono font-medium text-cyan-400">
                  {log.policyChecks.piiDetected === 0
                    ? "0 Detections"
                    : `${log.policyChecks.piiDetected} Redacted`}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-navy-900/80 border border-navy-800">
                ...
                <span
                  className={`font-mono font-medium ${log.policyChecks.budgetThresholdPassed ? "text-emerald-400" : "text-amber-400"}`}
                >
                  {log.policyChecks.budgetThresholdPassed
                    ? "✓ Compliant"
                    : "⚠ Near Cap"}
                </span>
              </div>
            ...
            {log.errorMessage && (
              <div className="p-3 bg-rose-950/40 border border-rose-800/60 rounded-lg text-xs font-mono text-rose-300">
                <span className="font-semibold uppercase tracking-wider text-rose-400 block mb-1">
                  Execution Failure:
                </span>
                {log.errorMessage}
              </div>
