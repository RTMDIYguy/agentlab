        ...
        <div className="flex items-start justify-between p-6 border-b border-navy-800/50 shrink-0">
          <div>
            ...
            <p className="font-inter text-sm text-slate-400 mt-1">
              Design a new autonomous execution chain.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto space-y-8 flex-1 custom-scrollbar">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-navy-800/50 pb-2">
              <label className="font-inter text-xs font-semibold text-cyan-400 uppercase tracking-widest">
                Execution Steps
              </label>
              <span className="text-xs text-slate-500 font-mono bg-navy-950 px-2 py-1 rounded border border-navy-800">
            ...
            <div className="space-y-3">
              {steps.map((step, index) => (
                <div
                  key={step.id}
                  className="relative flex gap-4 p-4 bg-navy-950/30 border border-navy-800/50 rounded-xl group hover:border-cyan-500/30 transition-colors"
                >
                  <div className="flex flex-col items-center mt-1">
                    ...
                    {index !== steps.length - 1 && (
                      <div className="w-px h-full bg-navy-800 mt-2"></div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="sm:col-span-2 space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full bg-navy-950 border border-navy-800 rounded-md text-white px-3 py-2.5 font-inter text-sm focus:outline-none focus:border-cyan-500 placeholder-slate-600 transition-colors"
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <select className="w-full bg-navy-950 border border-navy-800 rounded-md text-white px-3 py-2.5 font-inter text-sm focus:outline-none focus:border-cyan-500 appearance-none transition-colors">
                        ...
                        <option className="italic text-cyan-500">
                  ...
                  {steps.length > 1 && (
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 absolute -right-2 -top-2 bg-navy-800 hover:bg-rose-500 text-slate-400 hover:text-white p-1.5 rounded-full border border-navy-700 transition-all shadow-lg focus:outline-none"
                    >
                      <svg
            ...
            <Button
              variant="secondary"
              onClick={addStep}
              className="w-full border-dashed border-navy-700 hover:border-cyan-500/50 text-slate-400 hover:text-cyan-400 py-3 mt-4 bg-navy-950/20"
            >
              <svg
