...
import { AuditDetailSlideOver, type AuditLogItem } from './AuditDetailSlideOver';
...
const INITIAL_AUDIT_DATA: AuditLogItem[] = [
  ...
  },
  {
    ...
    responsePayload: JSON.stringify({
      itemsScraped: 42,
      rateLimitWarning: "Approaching origin server rate limit (90/100 requests/min)",
      retryAfterSeconds: 4
    }, null, 2),
    policyChecks: {
      saifPassed: true,
  ...
  },
  {
    ...
    errorMessage: 'PostgreSQL connection timeout during read-only ledger reconciliation query.',
    policyChecks: {
      saifPassed: false,
      piiDetected: 0,
      budgetThresholdPassed: true
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter(item => {
      ...
      const matchesSearch = 
        ...
        item.workflowContext.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="font-inter text-sm text-slate-400 mt-1">
            Real-time LLM trace monitoring, security guardrails, token consumption, and cost telemetry.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-navy-900 border border-navy-800/80 p-5 rounded-2xl">
          <div className="text-xs font-inter text-slate-400 mb-1">Today's Inference Spend</div>
          <div className="font-mono text-2xl font-bold text-emerald-400">${summaryStats.totalCost.toFixed(4)}</div>
          <div className="text-[11px] font-inter text-slate-500 mt-1">100% within daily cap ($50.00)</div>
        </div>

        <div className="bg-navy-900 border border-navy-800/80 p-5 rounded-2xl">
          <div className="text-xs font-inter text-slate-400 mb-1">Total Tokens Processed</div>
          <div className="font-mono text-2xl font-bold text-cyan-400">
            {summaryStats.totalTokens.toLocaleString()}
          </div>
          <div className="text-[11px] font-inter text-slate-500 mt-1">Across 4 active models</div>
        </div>

        <div className="bg-navy-900 border border-navy-800/80 p-5 rounded-2xl">
          <div className="text-xs font-inter text-slate-400 mb-1">Policy Guardrail Pass Rate</div>
          <div className="font-mono text-2xl font-bold text-white">{summaryStats.passRate}%</div>
          <div className="text-[11px] font-inter text-emerald-400 mt-1">✓ SAIF security active</div>
        </div>

        <div className="bg-navy-900 border border-navy-800/80 p-5 rounded-2xl">
          <div className="text-xs font-inter text-slate-400 mb-1">Logged Executions</div>
          <div className="font-mono text-2xl font-bold text-amber-400">{summaryStats.count} Traces</div>
          <div className="text-[11px] font-inter text-slate-500 mt-1">Filtered from buffer</div>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-navy-800/40 text-xs font-inter">
              {filteredLogs.length === 0 ? (
                <tr>
                  <td colSpan={9} className="py-12 text-center text-slate-500 font-inter">
                ...
                filteredLogs.map((item) => (
                  ...
                  >
                    <td className="py-4 px-5">
                      <div className="font-mono text-slate-300">{item.timestamp.split(' ')[1]}</div>
                      <div className="font-mono text-[10px] text-cyan-500">{item.id}</div>
                    </td>
